...
procedure TStrings_IndexError(Self: TStrings; AIndex, AMaxIndex: Integer);
var
  s: string;
begin
  if Self <> nil then
    ...
    s := '<nil>';
  raise EStringListError.CreateFmt(SListIndexErrorExt, [AIndex, AMaxIndex, s]) at ReturnAddress;
end;
...
begin
  if (Index1 < 0) or (Index1 >= FCount) then
    TStrings_IndexError(Self, Index1, FCount - 1);
  if (Index2 < 0) or (Index2 >= FCount) then
    TStrings_IndexError(Self, Index2, FCount - 1);
  Changing;
...
begin
  ...
  if (Index < 0) or (Index > FCount) then
    TStrings_IndexError(Self, Index, FCount);
...
begin
  if FAction = nil then
    Exit(False);
  LAct := FAction;
  LAct.ActionComponent := AComponent;
  try
    Result := LAct.Execute;
  finally
    LAct.ActionComponent := nil;
  end;
end;
